...
### TPC-H
The TPC Benchmark™H (TPC-H) is a standard decision support benchmark for big data analytics. It contains synthetic data and 22 default queries, representing a generalized complex analytics workload that answers critical business questions. The queries are intentionally made computational heavy, typically involving multiple joins, complex aggregations, and subqueries. The data size and distribution can be controlled via arguments while generating the synthetic data. Below is the schema of the benchmark database. The configuration and queries I used will be described in the next section.
...
### Data and Queries
To test out how Redshift scales on various data sizes, I used TPC-H datasets generated in sizes 10GB, 100GB, and 300GB, all with uniform data distributions. To see how data skew affects query performance, I also incorporated a skewed version of the 100GB dataset, generated using a Zipfian Distribution with theta = 1. All these data are published on S3: s3://uwdb/tpch and s3://tpch-300 (Special thanks for Shrainik Jain and Guna Prasaad for posting these online).

The queries I evaluated consist of both TPC-H queries (with constants in predicates randomly generated), and some simpler queries I made up. 2 out of the 22 queries (q4 and q20) contain correlated subquery patterns that are not supported by Redshift, and therefore were omitted in this experiment. The simpler queries I wrote contain scanning on predicates, aggregation and sorting (on 2-3 attributes), and simple multiway joins (3-5 way joins). I also limited the final number of rows returned by scan and joins.

### Cluster Configuration
Amazon provides specialized instance types for Redshift clusters. Although they are just EC2 instances, there are no direct equivalent instance types available in EC2. I used the default type "dc1.large" (which is covered by the Redshift free trial). Each such node contains 15GB memory, 2 cores and 160GB SSD. Its network IO performance is "moderate" by AWS's term. To see how redshift scales, I used 2, 6, and 18 compute nodes for comparison (so that's 4, 12, and 36 node slices, plus one leader node in each cluster). I used a growth factor of 3 since it helps with the evaluation of the scale-up and speed-up on the 100GB and 300GB TPC-H data.
...
Later I realized that I was not using the most efficient way of loading the data. I loaded the whole data file all at once, whereas it would be better to split it into smaller chunks to allow more parallelism and reduce the bottleneck at the master node.
...
All later results reported are for warm start only, where I preheated the database with 1 or 2 runs and measured the runtime of two subsequent queries and reported the average.
...
The figures above show the run time of different types of queries. A closer look at the TPC-H queries' performance shows that the total TPC-H's runtime is usually dominated by queries with complex multiway joins.

The Redshift dashboard provides detailed monitoring of CPU usages and disk/network IO during query execution. One observation I found is that the slowest queries are usually the ones that spill intermediate results to disk. The ones with in-memory execution are much faster on average. The disk usage shown below is for one of the slower TPC-H queries.
...
Speed up and scale up are two important metrics for a database's scalability. I first measured speed up by fixing the dataset size, and add more nodes to see if the query time reduces proportionally. First we notice that the speed up is highly query dependent - aggregates and joins see larger improvements. Also, the difference from 6 to 18 nodes is much smaller than growing from 2 to 6 nodes, showing diminishing return for adding more nodes.


![scaleup](https://i.imgur.com/6EG4pUw.png)

Then I increased the dataset size and number of compute nodes porportionally. We can see that scale up is slightly more consistent across query types, although also sub-linear. Note that queries involving complicated joins benefit less from the increased number of workers.
...
Here I measured the runtime of aggregates and scans on 10GB data (the others take longer). Reading from S3 is much slower as expected, but it could be sufficient for large static relations that are less frequently read from.


## Discussions
### Administrative workflow
I think Redshift succeeded in providing an easy-to-use administrative interface. Database snapshots are taken automatically at a regular basis, completely transparent to the database users (therefore they might affect the measurements in this experiment). Snapshots are saved in S3 and can be managed in the dashboard. Reconfiguration and snapshot restoration is rather straightforward in the console UI. However since it's a fully managed service, sometimes the user might have a hard time understanding the exact behaviors underlying the status changes in the console, especially during reconfigurations such as resizing and rebooting.
### Limitations and future work

The measurements in the experiment are very coarse-grained, and only aim to provide a high level intuition of Redshift's performance. For instance, I only measured the average of two runs per query after "preheating" with one run. As mentioned earlier, the way I load the data from S3 could be improved as well. Further things that could potentially be benchmarked includes operations on encrypted data, loading compressed data, and concurrent and transational query performance.

### Conclusion
Redshift works great out-of-the-box. It's very easy to get a Redshift cluster up and running on AWS (compared to other systems I tried this quarter). Also it fits nicely into AWS's ecosystem, and integrate closely with S3, especially with the new Spectrum service. The declarative SQL with Python UDF gives some degree of flexibility, but not at the level as a general purpose computation engine such as Spark. However the plus side is that it should be very easy for clients writing SQL to migrate to Redshift, from Amazon's own RDS, or other standard RDBMS. Under my configuration, Redshift shows near-linear scale-up and less-so-linear speed-up, and the its performance is highly query dependent.
